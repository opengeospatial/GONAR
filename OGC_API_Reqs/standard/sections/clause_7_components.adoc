...
[[needs-item]]
=== Requirements Class "Needs" (Component)

==== Overview

include::../requirements/requirements_class_needs_item.adoc[]

The Need Item requirements class defines how the GONAR conceptual model is combined with the Record data model described in section 7.2 of https://docs.ogc.org/is/20-004r1/20-004r1.html[OGC API – Records]. A _Need Item_ is the container of the actual needs.
...
[[fb_item_props]]
==== Properties
The properties of a record that contains needs uses the GONAR conceptual model for the Needs Item $$$as described in section 7.2 of OGC GONAR Standard: Conceptual Model$$$. The following table maps the properties defined in OGC API - Records section 7.2 with the GONAR Needs item. 
